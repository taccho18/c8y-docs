...
aliases:
  - /users-guide/administration/managing-users
---
...
> **Required permission:** User management ADMIN or CREATE

**Single sign-on**

If your tenant is configured for using single sign-on (SSO) in {{< sag-cloud >}}, you must create new users under **My Cloud**, accessible through the application switcher in the upper right corner, if you want them to be able to use the single sign-on feature.
...
> **Info:** Users who are using single sign-on cannot change the password of users who are managed by the platform.
...
To filter the list, you can use the search field at the left of the top menu bar. For details on the search functionality, refer to [Getting started > UI functionalities and features > Search and filter functionality](/users-guide/getting-started/#searching).
...
Follow these steps to create a new user in your tenant.
...
2. At the left of the **New user** window, provide the following information to identify the user:
	...
	<td align="left">Serves as a user ID to identify the user at the system. Note that the username cannot be changed once the user has been created. **Mandatory**</td>
	</tr>
	<tr>
	...
	<td align="left">A valid email address. This is required to enable the user to reset the password. **Mandatory**</td>
	</tr>
	<tr>
	...
	If your tenant is subscribed to the user hierarchies feature you will find two more fields. Refer to <a href="../../users-guide/enterprise-tenant#user-hierarchies">Managing user hierarchies</a> for details on user hierarchies.
	...
	<td align="left">Another user that manages ("owns") the new user. Select a user from the dropdown list and click <strong>Done</strong> to confirm. </td>
	</tr>
	<tr>
	<td align="left">Delegated by</td>
	<td align="left">Can be activated to delegate user hierarchies and permissions to the user.</td>
	</tr>
	</tbody>
	</table>
...
3. Select the login options for the user.
	...
	* **User must reset password on next login**: If selected, you need to provide a password which the user must reset on the next login. Enter a password and confirm it. While entering the password, the strength of the password will be checked. See [Getting started > Accessing and logging into the platform > To log into the {{< product-c8y-iot >}} platform](/users-guide/getting-started/#login) for further information on password strength.  
